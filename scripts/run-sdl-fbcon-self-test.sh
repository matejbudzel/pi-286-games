...
echo "Filling the 640x480 framebuffer blue for three seconds; this takes over the active tty."
exec env LD_LIBRARY_PATH="$prefix/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" SDL_VIDEODRIVER="${SDL_VIDEODRIVER:-fbcon}" SDL_FBDEV="${SDL_FBDEV:-/dev/fb0}" SDL_FB_BROKEN_MODES="${SDL_FB_BROKEN_MODES:-1}" "$binary"
