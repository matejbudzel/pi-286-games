...
pillarbox=$(host_setting dosbox_sdl_fb_pillarbox)
...
if [ -n "$boot_config" ]; then
    ...
    expected_width=$(host_setting framebuffer_width); [ -n "$expected_width" ] || expected_width=640
    expected_height=$(host_setting framebuffer_height); [ -n "$expected_height" ] || expected_height=480
    expected_depth=$(host_setting framebuffer_depth); [ -n "$expected_depth" ] || expected_depth=16
    expected_group=$(host_setting framebuffer_hdmi_group); [ -n "$expected_group" ] || expected_group=2
    expected_mode=$(host_setting framebuffer_hdmi_mode); [ -n "$expected_mode" ] || expected_mode=4
    for setting in hdmi_force_hotplug=1 hdmi_drive=2 hdmi_blanking=0 disable_overscan=1 "hdmi_group=$expected_group" "hdmi_mode=$expected_mode" "framebuffer_width=$expected_width" "framebuffer_height=$expected_height" "framebuffer_depth=$expected_depth" dtparam=audio=on; do grep -Eq "^[[:space:]]*$setting[[:space:]]*$" "$boot_config" && pass "boot setting $setting" || warn "boot setting missing: $setting"; done
    expected_cvt=$(host_setting framebuffer_hdmi_cvt)
    [ -z "$expected_cvt" ] || { grep -Eq "^[[:space:]]*hdmi_cvt[[:space:]]*=[[:space:]]*$expected_cvt[[:space:]]*$" "$boot_config" && pass "boot setting hdmi_cvt=$expected_cvt" || warn "boot setting missing: hdmi_cvt=$expected_cvt"; }
