...
if [ ! -x /opt/sdl12-fbcon/bin/sdl-config ] || [ "$(/opt/sdl12-fbcon/bin/sdl-config --version 2>/dev/null)" != 1.2.16 ] || [ ! -e /opt/sdl12-fbcon/lib/libSDL-1.2.so.0 ]; then
    echo "Missing deployed classic SDL at /opt/sdl12-fbcon. Cross-build and deploy it from the development machine before running this installer." >&2
    exit 1
fi
...
if [ -x /opt/sdl12-fbcon/bin/sdl-config ] && [ "$(/opt/sdl12-fbcon/bin/sdl-config --version 2>/dev/null)" = 1.2.16 ] && [ -e /opt/sdl12-fbcon/lib/libSDL-1.2.so.0 ]; then
    for setting in \
        ...
        'presenter_sdl_fb_broken_modes=1'; do
        key=${setting%%=*}
    ...
    grep -q '^presenter_sdl_fb_pillarbox=' "$repo/config/host.conf" || printf '%s\n' 'presenter_sdl_fb_pillarbox=0' >> "$repo/config/host.conf"
else
    echo "INFO: custom classic SDL is not installed; leaving presenter SDL settings unchanged."
fi
