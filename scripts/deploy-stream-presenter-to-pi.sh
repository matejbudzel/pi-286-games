...
ssh "$target" 'mkdir -p ~/.config /opt/pi286/stream/bin'
scp "$binary" "$target":/opt/pi286/stream/bin/pi286-stream-presenter
ssh "$proxmox" "pct exec $container -- cat /etc/pi286-stream.token" | ssh "$target" 'umask 077; cat > ~/.config/pi286-stream.token'
ssh "$target" 'chmod 700 /opt/pi286/stream/bin/pi286-stream-presenter; chmod 600 ~/.config/pi286-stream.token'
echo "Presenter deployed. Set dosbox_backend=auto and remote_dosbox_url=http://192.168.100.194:28680 in config/host.conf."
