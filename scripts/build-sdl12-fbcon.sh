...
valid_install() { [ -x "$prefix/bin/sdl-config" ] && [ -e "$prefix/lib/libSDL-1.2.so.0" ] && [ "$("$prefix/bin/sdl-config" --version 2>/dev/null)" = 1.2.16 ]; }
...
./autogen.sh
./configure --prefix="$prefix" --enable-audio --enable-audio-alsa --enable-video-fbcon --disable-video-x11 --disable-video-opengl
make -j"$jobs"
