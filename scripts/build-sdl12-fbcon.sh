...
valid_install() { [ -x "$prefix/bin/sdl-config" ] && [ -e "$prefix/lib/libSDL-1.2.so.0" ] && [ "$("$prefix/bin/sdl-config" --version 2>/dev/null)" = 1.2.16 ] && ldd "$prefix/lib/libSDL-1.2.so.0" 2>/dev/null | grep -q 'libasound\.so'; }
