...
object=${out}.o
"$cc" --sysroot="$sysroot" $flags -I"$stage/opt/sdl12-fbcon/include/SDL" -c "$repo/streaming/client/pi286-stream-presenter.c" -o "$object"
...
"$cc" --sysroot="$sysroot" $flags -pie -nostartfiles -nodefaultlibs \
  ...
  -L"$stage/opt/sdl12-fbcon/lib" -Wl,-rpath,/opt/sdl12-fbcon/lib -lSDL \
