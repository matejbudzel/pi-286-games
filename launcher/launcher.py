...
def write_dosbox_replay(dosbox, game_conf, generated_conf, environment):
    """Keep a runnable copy of the precise appliance launch beside the log."""
    replay = Path("/tmp/pi-286-games-dosbox-command.sh")
    variables = ("LD_LIBRARY_PATH", "SDL_VIDEODRIVER", "SDL_FBDEV", "SDL_FB_BROKEN_MODES", "SDL_AUDIODRIVER", "AUDIODEV")
    command = ["env"] + ["%s=%s" % (name, environment[name]) for name in variables if name in environment]
    command += [dosbox, "-conf", str(game_conf), "-conf", str(generated_conf)]
...
def run_game(game, config, term, pad, ddr_keys, no_sound=False):
    ...
    try:
        ...
        try:
            ...
            write_dosbox_replay(dosbox, game.dosbox_conf, generated_copy, environment)
