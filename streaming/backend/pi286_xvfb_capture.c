...
enum { OUTPUT_WIDTH = 320, OUTPUT_HEIGHT = 240, OUTPUT_BYTES = OUTPUT_WIDTH * OUTPUT_HEIGHT * 2,
       NATIVE_WIDTH = 640, NATIVE_HEIGHT = 480, NATIVE_BYTES = NATIVE_WIDTH * NATIVE_HEIGHT * 2 };
...
static int convert(const unsigned char *source, size_t source_size, const char *scaling, int native_2x,
    ...
    if (native_2x) {
        for (int y = 0; y < NATIVE_HEIGHT; ++y) {
            const unsigned char *row = source + pixels + (size_t)y * bytes_per_line;
            for (int x = 0; x < NATIVE_WIDTH; ++x) {
                const unsigned char *pixel = row + x * 4;
                uint16_t color = (uint16_t)(((pixel[2] & 0xf8) << 8) | ((pixel[1] & 0xfc) << 3) | (pixel[0] >> 3));
...
int main(int argc, char **argv) {
    ...
    if (argc != 3 && !native_2x) { fprintf(stderr, "usage: %s Xvfb_screen0 nearest|linear-v|crt-lite|tv2x|rgb2x [2x]\n", argv[0]); return 64; }
    unsigned char *source = NULL, output[NATIVE_BYTES];
    size_t source_size = 0;
    int result = stable_frame(argv[1], &source, &source_size);
    if (!result) result = convert(source, source_size, argv[2], native_2x, output);
    free(source);
    if (result) return result == 75 ? 75 : 65;
    size_t output_size = native_2x ? NATIVE_BYTES : OUTPUT_BYTES;
    return fwrite(output, 1, output_size, stdout) == output_size ? 0 : 74;
}
